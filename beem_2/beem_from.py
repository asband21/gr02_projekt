...
while True:
    ...
    for channel in range(ka):
        ...
        kernel_size = 1
        kernl = np.ones(kernel_size) / kernel_size
        recording[:, channel] = np.convolve(recording[:, channel], kernel, mode='same')

    #plt.subplot(1, 2, 1)
    #plt.plot(recording[:, channel], label=f"Kanal {channel+5}")
    
    for i in range(vinkel_afsnit):
        v = i*(2*math.pi/vinkel_afsnit)
        rol = [0,0,0,0]
        for j in range(ka):
            tid = (math.sin(v+j*math.pi/2))*afstan/(speed_sound*2)
            rol[j] = tid*samplerat;
        #print(rol[0])
        mid_spor = [0,0,0,0];
        for j in range(ka):
        ...
        mid_vadi[:,i] = (np.roll(mid_spor[0],int(rol[0]))- np.roll(mid_spor[1],int(rol[1])))*(np.roll(mid_spor[0],int(rol[0])) - np.roll(mid_spor[1],int(rol[1]))) + (np.roll(mid_spor[2],int(rol[2])) - np.roll(mid_spor[3],int(rol[3])))*(np.roll(mid_spor[2],int(rol[2])) - np.roll(mid_spor[3],int(rol[3])))
        for channel in range(ka):
            #plt.subplot(1, 2, 1)
